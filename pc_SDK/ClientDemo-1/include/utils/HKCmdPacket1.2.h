...
//
//    AFX_EXT_CLASS int compact_conv_bencode(char old[], size_t len, const char* buf, size_t bufsiz);
//
//#ifdef __cplusplus
//} //extern "C" //#if defined(_WIN32) #  define snprintf sprintf_s #endif

#include <string>
#include <map>
#include "utils/HKChameleon.h"
typedef std::map<std::string,std::string>  _HKKeyMap;

class AFX_EXT_CLASS HKCmdPacket
{
public:
	HKCmdPacket( void );
	...
	virtual ~HKCmdPacket( void );

	void SetParam( const std::string& strKEY,const char* value );
