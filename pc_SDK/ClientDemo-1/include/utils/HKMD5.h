...
class AFX_EXT_CLASS HKMD5
{
public:
	static unsigned long long MD5Encode(unsigned char* pBuf,unsigned int nLength);
	static unsigned long long MD5Encode(const std::string& buf);
	static std::string MD5Encode128Hex(unsigned char* pBuf, unsigned int nLength);
protected:
	HKMD5();
	virtual ~HKMD5(){};
